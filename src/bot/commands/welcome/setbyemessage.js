...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .setDescription('Set the goodbye message template (placeholders: {mention}, {user}, {server})')
  ...
  async execute(interaction){
    ...
    if (reset){
      moderation.setGuildConfig(interaction.guild.id, { byeMessage: null });
      const embed = new EmbedBuilder().setTitle('Goodbye message reset').setDescription('Goodbye message reset to default.').setColor(0x2ECC71);
    ...
    moderation.setGuildConfig(interaction.guild.id, { byeMessage: msg });
    const embed = new EmbedBuilder().setTitle('Goodbye message set').setDescription('Set the goodbye template. Use {mention} to insert the user mention.').setColor(0x2ECC71);
    return interaction.reply({ embeds: [embed], ephemeral: true });
  }
};