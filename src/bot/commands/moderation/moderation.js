    ...
    try {
      if (sub === 'warn'){
        ...
        if (!list || list.length === 0) {
          const { EmbedBuilder } = require('discord.js');
          return interaction.followUp({ embeds: [ new EmbedBuilder().setTitle('No warnings').setDescription(`No warnings for ${target.tag}`).setColor(0x95A5A6) ], flags: 64 });
        }
        ...
        if (!member.moderatable) {
          const { EmbedBuilder } = require('discord.js');
          return interaction.reply({ embeds: [ new EmbedBuilder().setTitle('Action failed').setDescription('I cannot time out this user.').setColor(0xE74C3C) ], ephemeral: true });
        }
        ...
        if (!ms) {
          const { EmbedBuilder } = require('discord.js');
          return interaction.followUp({ embeds: [ new EmbedBuilder().setTitle('Invalid duration').setDescription('Invalid duration format. Use 30m or 1h etc.').setColor(0xE74C3C) ], flags: 64 });
        }
        ...
        if (amount < 2 || amount > 100) {
          const { EmbedBuilder } = require('discord.js');
          return interaction.followUp({ embeds: [ new EmbedBuilder().setTitle('Invalid amount').setDescription('Amount must be between 2 and 100').setColor(0xE74C3C) ], flags: 64 });
        }
