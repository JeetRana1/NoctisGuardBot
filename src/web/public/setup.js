(function(){
  ...
  async function handleCreateGiveaway(){
    ...
    try {
      ...
      const res = await fetch('/api/guilds/' + guildId + '/giveaways', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prize,duration,winners,channelId,requireRole:role})});
  ...
  async function handleSetLvl(){ const rate = Number(document.getElementById('lvl-rate').value); const dur = parseInt(document.getElementById('lvl-duration').value,10); if(!rate || rate<=0) return alert('Please enter a positive rate like 2'); const body={rate}; if(dur && dur>0) body.durationMinutes = dur; const r = await fetch('/api/guilds/'+guildId+'/xprate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); if(r.ok){ alert('XP rate updated'); loadLeveling(); } else alert('Failed'); }
  async function handleResetLvl(){ if(!confirm('Reset XP rate to 1x for this server?')) return; const r = await fetch('/api/guilds/'+guildId+'/xprate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({reset:true})}); if(r.ok){ alert('XP rate reset'); loadLeveling(); } else alert('Failed'); }
  async function loadLeveling(){ const r = await fetch('/api/guilds/'+guildId+'/xprate'); if(!r.ok) return; const j = await r.json(); document.getElementById('lvl-current').textContent = (j.xpRate||1)+'x'; if(j.xpRateExpires) document.getElementById('lvl-current').textContent += ' (expires: '+new Date(j.xpRateExpires).toLocaleString()+')'; }

  async function handleSetLog(){ const btn = document.getElementById('btn-set-log'); if (!btn || btn.dataset.processing === '1') return; try { btn.dataset.processing = '1'; btn.disabled = true; const ch = document.getElementById('log-channel-input').value.trim(); if(!ch) return alert('Enter a channel ID'); const r = await fetch('/api/guilds/' + guildId + '/logchannel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({channelId:ch})}); if(r.ok){ alert('Log channel updated'); loadLogChannel(); } else alert('Failed'); } finally { const btn = document.getElementById('btn-set-log'); if (btn) { btn.disabled = false; delete btn.dataset.processing; } } }
  async function handleClearLog(){ const btn = document.getElementById('btn-clear-log'); if (!btn || btn.dataset.processing === '1') return; try { btn.dataset.processing = '1'; btn.disabled = true; if(!confirm('Clear the log channel?')) return; const r = await fetch('/api/guilds/' + guildId + '/logchannel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({clear:true})}); if(r.ok){ alert('Cleared'); loadLogChannel(); } else alert('Failed'); } finally { const btn = document.getElementById('btn-clear-log'); if (btn) { btn.disabled = false; delete btn.dataset.processing; } } }
  async function loadLogChannel(){ const r = await fetch('/api/guilds/'+guildId+'/logchannel'); if(!r.ok) return; const j = await r.json(); document.getElementById('log-channel-input').value = j.channelId || ''; }

  async function loadCases(){ const res = await fetch('/api/guilds/'+guildId+'/cases'); if(!res.ok) return; const json = await res.json(); const list = json.cases || []; const container = document.getElementById('cases-list'); container.innerHTML=''; list.forEach(c=>{ const el=document.createElement('div'); el.className='section-card'; const title=document.createElement('h4'); title.style.margin='0 0 8px'; title.textContent = '#'+c.id+' • '+c.type; const pid=document.createElement('p'); pid.style.color='var(--muted)'; pid.style.margin='0'; pid.textContent = 'Target: <@'+c.targetId+'>'; const status=document.createElement('p'); status.style.color='var(--muted)'; status.style.margin='6px 0 0'; status.textContent = (new Date(c.timestamp)).toLocaleString()+' • Reason: '+c.reason; el.appendChild(title); el.appendChild(pid); el.appendChild(status); container.appendChild(el); });
    try{
  ...
  async function handleSaveSettings(){
    ...
    try{
      ...
      const r = await fetch('/api/guilds/' + guildId + '/settings');
      ...
      const save = await fetch('/api/guilds/' + guildId + '/settings', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ settings }) });
